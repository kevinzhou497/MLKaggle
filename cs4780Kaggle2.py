from sklearn.model_selection import train_test_split
from sklearn.linear_model import LinearRegression
from sklearn import metrics
import pandas as pd
import pickle as pkl
import numpy as np

file = open('covid_dataset.pkl', 'rb')
checkpoint = pkl.load(file)
file.close()

# data = pd.read_pickle("covid_dataset.pkl")

# data = pd.DataFrame([data])
# # replace NaNs with mean
# data.fillna(0)
# data.replace([np.inf, -np.inf], 0)

# # data = data.astype(object)

# could just do numpy arrays?


X_train, y_train, X_val, y_val, X_test = checkpoint["X_train"], checkpoint[
    "y_train_log_pos_cases"], checkpoint["X_val"], checkpoint["y_val_log_pos_cases"], checkpoint["X_test"]

# find the mean of certain features

feature_means_train = []
for i in range(23):
    i_sum = 0
    for x in range(len(X_train)):
        if not X_train[x][i] == None:
            i_sum += X_train[x][i]
    feature_means_train.append(i_sum / len(X_train))

feature_means_val = []
for i in range(23):
    i_sum = 0
    for x in range(len(X_val)):
        if not X_val[x][i] == None:
            i_sum += X_val[x][i]
    feature_means_val.append(i_sum / len(X_val))


feature_means_test = []
for i in range(23):
    i_sum = 0
    for x in range(len(X_test)):
        if not X_test[x][i] == None:
            i_sum += X_test[x][i]
    feature_means_test.append(i_sum / len(X_test))


x_train = []
for x in (X_train):
    for i in range(23):
        if x[i] == None:
            x[i] = feature_means_train[i]
    x_train.append(x)

x_val = []
for x in X_val:
    for i in range(23):
        if x[i] == None:
            x[i] = feature_means_val[i]
    x_val.append(x)

x_test = []
for x in X_test:
    for i in range(23):
        if x[i] == None:
            x[i] = feature_means_test[i]
    x_test.append(x)

LR = LinearRegression()
LR.fit(x_train, y_train)
y_pred = LR.predict(x_val)
test_pred = LR.predict(x_test)

pd.DataFrame(test_pred).to_csv("predictions.csv",
                               header=["cases"], index_label="id")
